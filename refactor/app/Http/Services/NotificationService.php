...
use App\Http\Actions\Mailer\Mailer;
use DTApi\Http\Actions\JobLogger;
...
use DTApi\Http\Traits\TagStringFromArray;
use DTApi\Http\Traits\ToHourMinute;
...
class NotificationService {
    use ToHourMinute, TagStringFromArray;

    /**
     * Resend SMS Notifications to the Translators
     * @param Request $request
     * 
     */
    public function resendSMSNotifications(Request $request): array {
        $data = $request->all();
        $job = Job::find($data['jobid']);
        $job_data = (new JobService)->jobToData($job);
     ...
     * @return int
     */
    public function sendSMSNotificationToTranslator(Model $job): int {
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        foreach ($users as $oneUser) {
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                if (Push::isNeedToSendPush($oneUser->id)) continue;
                $not_get_emergency = TeHelper::getUsermeta($oneUser->id, 'not_get_emergency');
                if ($data['immediate'] == 'yes' && $not_get_emergency == 'yes') continue;
                $jobs = (new JobService)->getPotentialJobIdsWithUserId($oneUser->id); // get all potential jobs of this user
                    ...
                    if ($job->id == $oneJob->id) { // one potential job is the same with current job
                        ...
                        if ($job_for_translator == 'SpecificJob') {
                            ...
                            if (($job_checker != 'userCanNotAcceptJob')) {
                                if (Push::isNeedToDelayPush($oneUser->id)) {
        ...
        (new JobLogger)->addInfo($job_id, [$users, $data, $msg_text, $is_need_delay]);
     ...
     * @return mixed
     */
    public function getPotentialTranslators(Model $job): Collection {
    ...
    public function sendPushNotificationToSpecificUsers(
        ...
        $logger->addInfo($job_id, [$users, $data, $msg_text, $is_need_delay]);
    ...
    /**
     * Send notification on cancel job
     * @param int $job_id
     */
    public function sendNotificationByAdminCancelJob(int $job_id): array
    {
    ...
    public function sendChangedDateNotification($job, $old_time)
    {
        ...
        $mailer->send($name, $subject, 'emails.job-changed-date', $data);
        ...
        $mailer->send($translator->email, $translator->name, $subject, 'emails.job-changed-date', $data);
    }
